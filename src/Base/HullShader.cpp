...
namespace ml
{
	bool HullShader::LoadFromMemory(ml::Window& wnd, const char* code, ml::UInt32 codeLen, std::string entry, bool needsCompile)
	{
		mWindow = &wnd;
		...
		if (needsCompile) {
			...
			// compile
			HRESULT hr = D3DCompile(code, codeLen, "HullShader", nullptr, nullptr, entry.c_str(), "hs_5_0", flags, 0, outCode.GetAddressOf(), outMsgs.GetAddressOf());
			if (FAILED(hr)) {
