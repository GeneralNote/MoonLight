...
namespace ml
{
	bool DomainShader::LoadFromMemory(ml::Window& wnd, const char* code, ml::UInt32 codeLen, std::string entry, bool needsCompile, const Shader::MacroList& macros)
	{
		mWindow = &wnd;
		...
		if (needsCompile) {
			...
			// compile
			HRESULT hr = D3DCompile(code, codeLen, "DomainShader", macros.GetData(), nullptr, entry.c_str(), "ds_5_0", flags, 0, outCode.GetAddressOf(), outMsgs.GetAddressOf());
			if (FAILED(hr)) {
